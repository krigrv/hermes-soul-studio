...
const RULES = [
  ...
  },
  {
    ...
    message: "No companion files selected. That's fine — the operator still works on its own.",
  },
  {
    id: "no-username",
    level: "warn",
    test: (c) => !c.userName || c.userName.trim().length < 2,
    message: "Add your name in Step 1 so the operator knows who it serves.",
  },
  {
    id: "placeholder-name",
    level: "info",
    test: (c) => /your name|your company/i.test(c.userName || ""),
    message: "Your name still looks like a placeholder. Personalize it before exporting.",
  },
];
